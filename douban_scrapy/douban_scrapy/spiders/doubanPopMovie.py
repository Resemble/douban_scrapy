...
class DoubanMovieSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        movie_release_dates = info_dict.get("上映日期", "").replace("\t", " ") if "上映日期" in info_dict else info_dict.get("首播",
                                                                                                            "").replace(
            "\t", " ")
        movie_languages = info_dict.get("语言", "").replace("\t", " ")
        movie_runtime = info_dict.get("片长", "").replace("\t", " ") if "片长" in info_dict else info_dict.get("单集片长", "").replace(
                "\t", " ")
        movie_another_names = ""
